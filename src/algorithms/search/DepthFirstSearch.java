...
public class DepthFirstSearch extends ASearchingAlgorithm  {
    ...
    public Solution solve(ISearchable domain) {
        if(domain==null)
            return null;
        PriorityQueue<AState> cells= new PriorityQueue<>();
        domain.ResetVisit();//reset all cells to false, to know if been visited
        cells.add(domain.getStartState());// get the first pos of the maze to the queue
        domain.
...
//            }
//            MyNeighbours = domain.getAllSuccessors(temp); //get all possible states from current state
