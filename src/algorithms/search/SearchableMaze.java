...
public class SearchableMaze implements ISearchable {
    ...
    private ArrayList<AState> getPossibleStates(int x, int y)
    {
     return null;
    }
    ...
    private ArrayList<AState> getAllPossibleStates(int x, int y) {
        ArrayList<AState> temp = new ArrayList<AState>();
        MazeState tempM;
        if (isLegal(x - 1, y - 1) && visitedCell[x - 1][y - 1] == false && maze.getCellValue(x - 1, y - 1) == "0")
        ...
        if (isLegal(x - 1, y + 1) && visitedCell[x - 1][y + 1] == false && maze.getCellValue(x - 1, y + 1) == "0")
        {
            if (isLegal(x-1,y)&& visitedCell[x - 1][y] == false&&maze.getCellValue(x - 1, y) == "0" || isLegal(x,y+1)&& visitedCell[x][y+1] == false&&maze.getCellValue(x, y + 1) == "0")
            {
                tempM = new MazeState(x - 1, y - 1);
                tempM.setCost(1.5);
                temp.add(tempM);
...
//TODO Need to take care of cost
    ...
    public ArrayList<AState> getAllSuccessors(AState s) {
        ...
        if (s != null && s instanceof MazeState) //make sure State is a MazeState
        {
            ...
            if (TempAdd != null)
                temp.add(TempAdd);
            tempD = getAllPossibleStates(x, y); //get all diagonal states
